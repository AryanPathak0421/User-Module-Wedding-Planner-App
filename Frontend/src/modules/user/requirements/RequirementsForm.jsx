...
const RequirementsForm = () => {
  ...
  return (
    <div className="min-h-screen pb-24" style={{ backgroundColor: theme.semantic.background.primary }}>
      ...
      {/* Fixed CTA Button - Start Wedding Planning */}
      <div className="fixed bottom-32 left-4 right-4 z-50 md:bottom-28">
        <button
          onClick={() => navigate('/user/wedding-form')}
          className="w-full py-4 px-6 rounded-xl font-bold text-white text-base shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2"
          style={{
            background: `linear-gradient(135deg, ${theme.colors.secondary[500]}, ${theme.colors.secondary[600]})`,
            boxShadow: `0 8px 24px ${theme.colors.secondary[500]}50`
          }}
        >
          <Icon name="calendar" size="sm" className="text-white" />
          <span>Start Wedding Planning</span>
        </button>
      </div>

      {/* Sticky Continue Button */}
      <div 
        className="fixed bottom-16 left-0 right-0 p-4 border-t safe-area-pb md:bottom-6"
        style={{ 
